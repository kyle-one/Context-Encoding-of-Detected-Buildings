...
def get_logger():
    ...
    fHandler = logging.FileHandler("{}/{}.log".format(config.WORK_DIR+'/log','9_16_SLEDDing_just_tail,reverse'), mode='w')
...
if __name__ == '__main__':
    ...
    #连接全节点
    logger.info('--------------------SLEDDing_just_tail,reverse--------------------')
    RNNs=['RNN','LSTM','GRU']
    DETECTIONs=['cascade_rcnn_r101','cascade_rcnn_r50','faster_rcnn_r101','faster_rcnn_r50']
    BIDIRECTIONALs=['True','False']
    FUNs=['None','multi_task','Spatial_Layout_Encoding']
    ...
    for i in BIDIRECTIONALs:
        list_result1["[RNN={},DETECTION={},BIDIRECTIONAL={},FUN={}]".format('RNN','cascade_rcnn_r101',i,'Spatial_Layout_Encoding')]=[]
        for b in range(5):
            logger.info("[RNN={},DETECTION={},BIDIRECTIONAL={},FUN={}]".format('RNN','cascade_rcnn_r101',i,'Spatial_Layout_Encoding'))
            ...
            model=RNN(8,16,4,2,i,'RNN').to(config.device)
            ...
            list_result1["[RNN={},DETECTION={},BIDIRECTIONAL={},FUN={}]".format('RNN','cascade_rcnn_r101',i,'Spatial_Layout_Encoding')].append(result)
            logger.info(str(maxtir))
            logger.info(str(F1(maxtir))+'\n')
    list_result['SLEDDing_just_tail_reverse']=list_result1
    with open(config.WORK_DIR+'/9_16_SLEDDing_just_tail_reverse.json', 'w+') as f:
