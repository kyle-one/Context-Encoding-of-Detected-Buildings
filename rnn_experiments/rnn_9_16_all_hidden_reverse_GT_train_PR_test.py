...
def get_logger():
    ...
    fHandler = logging.FileHandler("{}/{}.log".format(config.WORK_DIR+'/log','all_hidden_reverse_GT_train_PR_test'), mode='w')
...
if __name__ == '__main__':
    ...
    #连接全节点
    logger.info('-------------------- 连接全节点,reverse,GT train  PR test--------------------')
    RNNs=['RNN','LSTM','GRU']
    DETECTIONs=['cascade_rcnn_r101','cascade_rcnn_r50','faster_rcnn_r101','faster_rcnn_r50']
    BIDIRECTIONALs=['True','False']
    FUNs=['None','multi_task','Spatial_Layout_Encoding']
    ...
    list_result1["[RNN={},DETECTION={},BIDIRECTIONAL={},FUN={}]".format('RNN','cascade_rcnn_r101','False','Spatial_Layout_Encoding')]=[]
    for b in range(5):
        logger.info("[RNN={},DETECTION={},BIDIRECTIONAL={},FUN={}]".format('RNN','cascade_rcnn_r101','False','Spatial_Layout_Encoding'))
        ...
        model=RNN2(8,16,4,2,'False','RNN').to(config.device)
        ...
        list_result1["[RNN={},DETECTION={},BIDIRECTIONAL={},FUN={}]".format('RNN','cascade_rcnn_r101','False','Spatial_Layout_Encoding')].append(result)
        logger.info(str(maxtir))
        logger.info(str(F1(maxtir))+'\n')
    list_result['all_hidden_reverse_GT_train_PR_test']=list_result1
    with open(config.WORK_DIR+'/all_hidden_reverse_GT_train_PR_test.json', 'w+') as f:
