...
class config:
    ...
    PATH_GT_TRAIN = "/Public/TianYu/YongkunLiu/Datasets/BEAUTY1.0_COCOLike_8_16/Split0/annotations/train_reb.json"

    # PATH_PKL_TEST="//Public/YongkunLiu/workdir_coco_reb_new/faster_rcnn_r101_fpn_2x_coco/results_test.pkl"
    PATH_GT_TEST = "/Public/TianYu/YongkunLiu/Datasets/BEAUTY1.0_COCOLike_8_16/Split0/annotations/test.json"

    # PATH_PKL_VAL="Public/YongkunLiu/workdir_coco_reb_new/faster_rcnn_r101_fpn_2x_coco/results_val.pkl"
    PATH_GT_VAL = "/Public/TianYu/YongkunLiu/Datasets/BEAUTY1.0_COCOLike_8_16/Split0/annotations/val.json"

    PATH_PR = {'faster_rcnn_r50': '/Public/TianYu/YongkunLiu/workdir_coco_9_2/faster_rcnn_r50_fpn_2x_coco/',
               'cascade_rcnn_r101': '/Public/TianYu/YongkunLiu/workdir_coco_9_2/cascade_rcnn_r101_fpn_20e_coco/',
               'faster_rcnn_r101': '/Public/TianYu/YongkunLiu/workdir_coco_9_2/faster_rcnn_r101_fpn_2x_coco/',
               'cascade_rcnn_r50': '/Public/TianYu/YongkunLiu/workdir_coco_9_2/cascade_rcnn_r50_fpn_20e_coco/'}

    # PATH_PKL_TEST="/Public/YongkunLiu/workdir_coco_reb_new/faster_rcnn_r101_fpn_2x_coco/results_test.pkl"
    # PATH_GT_TEST="/Public/YongkunLiu/Datasets/BEAUTY1.0_COCOLikeReb/Split0/annotations/test.json"

    PATH_CLASSIFICATION = '/Public/TianYu/YongkunLiu/Datasets/BEAUTY1.0_COCOLike/classification.json'
    ...
    WORK_DIR = '/Public/TianYu/YongkunLiu/beauty_attention_work_dir_3_3'
    BATCH_SIZE = 64
    ...
    def spaceSort(image):  # 最大的排前面，然后按照与最大的距离排序，但是不插空
        ...
        for i in range(len(image) - 1):
            for j in range(len(image) - 1 - i):
                if (SpaceSort.getDistance(max_bbox['bbox'], image_sorted[j]['bbox']) > SpaceSort.getDistance(
                        max_bbox['bbox'], image_sorted[j + 1]['bbox'])):
                    t = image_sorted[j]
            ...
            if (idx == 0):  # 第一个直接添加到vis
                ...
                for visiteds_idx in range(len(vis)):
                    for visited_idx in range(len(vis[visiteds_idx])):
                        if (SpaceSort.iou_small(vis[visiteds_idx][visited_idx]['bbox'],
                                                i['bbox']) > 0.8):  # 有包含的，则加入list
...
def List2OneHot(image_list, fun, length, matrix=False, merge=False,
                onehot='confidence'):  # 将一个image的信息转为10*8的onehot fun表示排序方式。
...
def get_logger():
    ...
    fHandler = logging.FileHandler("{}/{}.log".format(config.WORK_DIR + '/log', 'all_hidden_reverse_GT_train_PR_test'),
                                   mode='w')
...
class Self_Attn2(nn.Module):
    ...
    def __init__(self, ):
        ...
        self.query_conv = nn.Linear(25 * 8, 25 * 8)
        self.key_conv = nn.Linear(25 * 8, 25 * 8)
        self.value_conv = nn.Linear(25 * 8, 25 * 8)
        self.gamma = nn.Parameter(torch.zeros(1))
...
class SA(nn.Module):
    ...
    def forward(self, input, tt):
        ...
        input = input.reshape([-1, 1, 25, 8])  # 卷积
        output1, hidden = self.att1(input)
        output2 = output1.reshape([-1, 200])
...
def train_multi_task(net, train_iter, val_iter, num_epoch, lr_period, lr_decay):
    ...
    for epoch in range(num_epoch):
        ...
        if not os.path.exists('./params'):
            os.makedirs('./params')
        logger.info(
            "[epoch {}][{}][end] train_loss={:.5f},loss_classfication={:.5f},loss_confidence={:.5f},train_acc={:.5f}({}/{})".format \
                (epoch, 'train', train_l_sum / (bidx + 1), loss_classfication_sum / (bidx + 1),
                 loss_confidence_sum / (bidx + 1), train_pred_sum / n, int(train_pred_sum), n))
...
def F1(maxtir):
    ...
    sum_Pre /= 4
    sum_Re /= 4
    return {'Pression': sum_Pre, 'Recall': sum_Re, 'F1': (2 * sum_Pre * sum_Re) / (sum_Pre + sum_Re),
            'Acc': sum_TP / sum_all}
...
if __name__ == '__main__':
    ...
    logger.info('-------------------- reverse,GT train  PR test--------------------')
    #     RNNs=['RNN','LSTM','GRU']
    DETECTIONs = ['cascade_rcnn_r101', 'cascade_rcnn_r50', 'faster_rcnn_r101', 'faster_rcnn_r50']
    #     BIDIRECTIONALs=['True','False']
    FUNs = ['None', 'multi_task', 'Spatial_Layout_Encoding']
    ...
    list_result1["[DETECTION={},FUN={}]".format('cascade_rcnn_r101', 'Spatial_Layout_Encoding')] = []
    for b in range(5):
        logger.info("[DETECTION={},FUN={}]".format('cascade_rcnn_r101', 'Spatial_Layout_Encoding'))
        ...
        train_ds = BEAUTY_bbox_Dataset(train_pr_list, 'None', 25, onehot='confidence')
        val_ds = BEAUTY_bbox_Dataset(val_pr_list, 'None', 25, onehot='confidence')
        test_ds = BEAUTY_bbox_Dataset(test_pr_list, 'None', 25, onehot='confidence')
        ...
        list_result1["[DETECTION={},FUN={}]".format('cascade_rcnn_r101', 'Spatial_Layout_Encoding')].append(result)
        logger.info(str(maxtir))
        logger.info(str(F1(maxtir)) + '\n')
    list_result['attention_reverse_GT_train_PR_test'] = list_result1
    with open(config.WORK_DIR + '/attention_reverse_GT_train_PR_test.json', 'w+') as f:
