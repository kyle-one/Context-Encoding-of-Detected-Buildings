...
def get_logger():
    ...
    fHandler = logging.FileHandler("{}/{}.log".format(config.WORK_DIR + '/log', '9_16_all_hidden.log'), mode='w')
...
class MLP(nn.Module):
    ...
    def __init__(self, inputSize, outputSize):
        ...
        self.linear = nn.Sequential(
            nn.Linear(inputSize, inputSize // 2),  # 200->100
            nn.ReLU(inplace=True),
            nn.Linear(inputSize // 2, inputSize // 4),  # 100->25
            nn.ReLU(inplace=True),
            nn.Linear(inputSize // 4, outputSize)  # 25->4
        )
...
if __name__ == '__main__':
    ...
    for b in range(5):
        ...
        model = MLP(200, 4).to(config.device)
    ...
    with open(config.WORK_DIR + '/3_3.json', 'w+') as f:
