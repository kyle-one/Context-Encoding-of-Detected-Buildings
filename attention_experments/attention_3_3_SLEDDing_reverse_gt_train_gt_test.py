...
def get_logger():
    ...
    fHandler = logging.FileHandler(
        "{}/{}.log".format(config.WORK_DIR + '/log', 'SLEDDing_all_hidden_reverse_GT_train_GT_test'), mode='w')
...
if __name__ == '__main__':
    ...
    # 连接全节点
    logger.info('-------------------- 空间排序  GT train  GT test--------------------')
    #     RNNs=['RNN','LSTM','GRU']
    DETECTIONs = ['cascade_rcnn_r101', 'cascade_rcnn_r50', 'faster_rcnn_r101', 'faster_rcnn_r50']
    #     BIDIRECTIONALs=['True','False']
    FUNs = ['None', 'multi_task', 'Spatial_Layout_Encoding']
    ...
    for b in range(5):
        ...
        val_pr_list = get_gt_list(config.PATH_GT_VAL)
        test_pr_list = get_gt_list(config.PATH_GT_TEST)
    ...
    list_result['SLEDDing_reverse_GT_train_GT_test'] = list_result1
    with open(config.WORK_DIR + '/SLEDDing_reverse_GT_train_GT_test.json', 'w+') as f:
